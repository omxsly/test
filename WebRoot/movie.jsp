...
<div id="page">
 ...
 <th class="s-amount">类型<hr/></th>
 <th class="s-agio">时长<hr/></th>
 <th class="s-total">上架<hr/></th>
 </tr>
 </thead>
...
<tr class="shop blue-line">  
 ...
 <td class="s-price">
 <c:choose>
      <c:when test="${list.schedule.state == 0}">
          已下映/<input type="submit" onclick="window.location.href='publishment.jsp?mid=${list.mid}&name=${list.name}'" value="重新上映" >
    
     </c:when> 
     <c:when test="${list.schedule.state == 1}">
         <input type="submit" onclick="window.location.href='publishment.jsp?mid=${list.mid}&name=${list.name}'" value="添加场次" >
     </c:when> 
     <c:otherwise>
        <input type="submit" onclick="window.location.href='publishment.jsp?mid=${list.mid}&name=${list.name}'" value="上映" >
     </c:otherwise>
     </c:choose>
</td>
